...
pecas_disponiveis = pecas_df[['Marca', 'Produto', 'Código da Peça']].to_dict(orient='records')
...
@app.route('/cadastro_peca', methods=['POST'])
def cadastrar_peca():
    """Endpoint para cadastrar uma peça selecionada"""
    data = request.json
    ...
    # Registrar a peça (aqui apenas simulado com uma mensagem)
    return jsonify({
